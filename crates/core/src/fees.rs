...
//! - price=0.10: ~0.56 (tokens)
//! - price=0.25: ~1.17
//! - price=0.50: ~1.56 (peak)
//! - price=0.75: ~1.17
//! - price=0.90: ~0.56
//!
...
impl FeeModel {
    ...
    pub fn calculate_fee(&self, price_tick: Tick, size: Size) -> Size {
        ...
        // fee = rate × tick × complement × size / (16000 × 10000 × 10000)
    ...
    pub fn effective_rate_bps(&self, price_tick: Tick) -> u16 {
        ...
        //
        // effective_rate = rate × (1 - price)
        let complement = (10000 - price_tick) as f64 / 10000.0;
        let rate = self.fee_rate_bps as f64 / 16000.0;
        (rate * complement * 10000.0).round() as u16
    }
...
mod tests {
    ...
    fn test_fee_at_50_percent() {
        ...
        assert_eq!(fee, 1_562_500); // 1.5625 shares
    }
